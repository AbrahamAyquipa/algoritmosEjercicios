...
public:
	...
	void move(Graphics^ g) {
		if (direction == 'S' && y + height * 0.03 <= g->VisibleClipBounds.Height) y += dy;
		if (direction == 'W' && y >= 0) y -= dy;
		if (direction == 'A' && x >= 0) x -= dx;
		if (direction == 'D' && x + width * 0.03 <= g->VisibleClipBounds.Width) x += dx;
	}
